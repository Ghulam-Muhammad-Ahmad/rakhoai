...
import { SessionsBulkTable } from "@/components/dashboard/SessionsBulkTable";
...
export default async function SessionsPage() {
  ...
  // events, surface the per-student attendance summary stored on Student. A
  // cheap count decides table-vs-fallback; the table fetches its own pages.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { count, error } = await (sb as any)
    .from("Session")
    .select("id", { count: "exact", head: true })
    .eq("academyId", dbUser.academy.id) as { count: number | null; error: { message: string } | null };

  const hasAnyEvents = (count ?? 0) > 0;
